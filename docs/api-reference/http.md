...
| `GET` | `/info` | `ServerInfoModel` | Server identity and capabilities: `{status, name, version}` plus langgraph-api's `langgraph_py_version`, `flags` and `host` — LangGraph Studio reads `flags` to enable in-app tracing. |
...
| `langsmith_tracer` | `LangSmithTracerModel` | `null` | `{project_name, example_id}` — also trace the run into that LangSmith project (Studio sets it). Ignored when tracing is off. |
